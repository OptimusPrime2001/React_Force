...
export default function EditUserManagement(props) {
  ...
  const [deskLookup, setDeskLookup] = useState();
  ...
  useEffect(() => {
    ...
    userManagementAction.GetDetailUserManagement(userId).then((res) => {
      ...
      setDeskSelected({
        id: res.content.deskId,
        value: res.content.deskId,
        label: res.content.deskName,
      })
    })
    getLookupScanner().then(res => {
      if (res && res.content) {
        setDeskLookup(res.content.map((item) => {
  ...
  const onSubmit = (data) => {
    ...
    formData.append("RoleId", userModel.roleId);
    deskSelected && formData.append("DeskId", deskSelected.id);
    selected.map((item) => {

      formData.append("Roles", item.label);
    })
    selectedDate && formData.append("DateOfBirth", selectedDate);
    ...
    files &&
      ...
      data.chatId && formData.append("chatId", data.chatId);
      data.teleToken && formData.append("teleToken", data.teleToken);

    data.content && formData.append("Description", data.content);
  ...
  const handleOnChangeRole = (data) => {
    ...
    if (data) {
      ...
      data.map((item) => {
        listData.push({ label: item.label, value: item.value })
      })
  ...
  const handleOnChangeDesk = (data) => {
    console.log(data);
    setDeskSelected(data);
  };
            ...
            <DialogContent className="pt-4 pb-2">
              ...
              <div className="form-group">
                <div className="row">
                  ...
                  <div className="col-12 col-md-6 col-lg-6 mb-3">
                    <label className="text-dark">Bàn làm việc</label>
                    <br />
                    <Select
                      className="basic-single mb-1"
                      ...
                      name="desk"
                      // defaultValue={deskSelected}
                      value={deskSelected}
                      // isMulti
                      onChange={handleOnChangeDesk}
                      options={deskLookup}
                      noOptionsMessage={() => "không tồn tại"}
                    />
                  </div>
              ...
              <div className="form-group">
                <div className="row">
                  <div className="col-12 col-md-6 col-lg-6 mb-3">
                    <label className="text-dark">TeleToken</label>
                    <TextField
                      inputRef={register}
                      defaultValue={userModel.teleToken}
                      type="text"
                      name="teleToken"
                      className="w-100"
                    />
                  ...
                  <div className="col-12 col-md-6 col-lg-6 mb-3">
                    <label className="text-dark">ChatId</label>
                    <TextField
                      inputRef={register}
                      defaultValue={userModel.chatId}
                      type="text"
                      name="chatId"
                      className="w-100"
                    />
