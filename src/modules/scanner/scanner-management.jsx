...
import { GetQrByScanner } from "../../redux/store/qr/qr.store";
import { connect } from "react-redux";
import { bindActionCreators } from "redux";
import ListScanner from "./list-scanner";
...
import CreateUpdateScanner from "./create-update-scanner";
import ModalQr from "../../components/modal-qr/modal-qr";

//--- Material Icon
import AddCircle from "@material-ui/icons/AddCircle";

const ScannerManagement = (props) => {
    ...
    const GetListAll = (params) => {
        showLoading(true);
        scannerAction.GetAll(params).then(res => {
            if (res && res.content) {
    ...
    const handleOpenGetQr = (id) => {
        onGetQr(id);
        setOpenGetQr(true);
    };

    const handleCloseGetQr = () => {
        setOpenGetQr(false);
    };
    ...
    const handleDelete = () => {
        showLoading(true);
        scannerAction.DeleteScanner(selectedId)
            .then(res => {
                if (res) {
                    getData();
...
export default connect(mapStateToProps, mapDispatchToProps)(ScannerManagement);